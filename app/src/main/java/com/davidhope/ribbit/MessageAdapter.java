...
public class MessageAdapter extends ArrayAdapter<ParseObject> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (message.getString(ParseConstants.KEY_FILE_TYPE).equals(ParseConstants.TYPE_IMAGE)){
            holder.iconImageView.setImageResource(R.drawable.ic_action_image_crop_original);
        }
        else if (message.getString(ParseConstants.KEY_FILE_TYPE).equals(ParseConstants.TYPE_VIDEO)){
            holder.iconImageView.setImageResource(R.drawable.ic_action_av_play_circle_outline);
        }
